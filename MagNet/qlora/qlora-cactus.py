...
OUTPUT_DIR = "llama3-finetuned-cactus"

wandb.login(key="wandb_key")

wandb.init(project="qlora-llama3-dialogue", name="llama3-8b-instruct-qlora-cactus", job_type="training")
...
prompt_text = load_prompt(f"cactus-dialogue-gen.txt")
...
def preprocess_cactus_function(examples):
    formatted_examples = []
...
dataset_cactus = load_dataset("json", data_files={"train":"datasets/cactus-train.json","test":"datasets/cactus-val.json"}, field="data")

# Shuffle Dataset
shuffled_dataset = DatasetDict({
    "train": dataset_cactus["train"].shuffle(seed=42),
    "test": dataset_cactus["test"].shuffle(seed=42)
})

...
OUTPUT_DIR = "llama3-finetuned-magnet"

wandb.login(key="wandb_key")

wandb.init(project="qlora-llama3-dialogue", name="llama3-8b-instruct-qlora-magnet", job_type="training")
...
prompt_text = load_prompt(f"magnet-dialogue-gen.txt")
...
def preprocess_magnet_function(examples):
    formatted_examples = []
    for i in range(len(examples['history'])):
        prompt = prompt_template.format(
            client_information=examples['client_information'][i],
            reason_counseling=examples['reason_counseling'][i],
...
dataset_magnet = load_dataset("json", data_files={"train":"datasets/magnet-train.json","test":"datasets/magnet-val.json"}, field="data")

# Shuffle Dataset
shuffled_dataset = DatasetDict({
    "train": dataset_magnet["train"].shuffle(seed=42),
    "test": dataset_magnet["test"].shuffle(seed=42)
})
